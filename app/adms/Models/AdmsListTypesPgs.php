...
class AdmsListTypesPgs
{
    ...
    public function listTypesPgs(int $page = null): void
    {
        ...
        $pagination = new AdmsPagination(URLADM . 'list-types-pgs/index');
        $pagination->condition($this->page, $this->limitResult);
        $pagination->pagination("SELECT COUNT(tp.id) AS num_result FROM adms_types_pgs tp");
        $this->resultPg = $pagination->getResult();

        $listUsers = new AdmsRead();
        $listUsers->fullRead(
            "SELECT id, type, name, order_type_pg, obs
                                FROM adms_types_pgs
                                ORDER BY id DESC
