...
/**
 * Solicitar link para cadastrar nova senha
 */
class AdmsRecoverPassword
{
    ...
    //Recebe o valor que sera inserido no banco de dados
     ...
     * @return void
     */
    public function recoverPassword(array|null $data = null): void
    {
        $this->data = $data;
    ...
    {
        $this->dataSave['recover_password'] = password_hash(date("Y-m-d H:m:s") . $this->resultBd[0]['id'], PASSWORD_DEFAULT);
    ...
    private function sendEmail(): void
    {
        ...
        if ($sendEmail->getResult()) {
            $_SESSION['msg'] = "<p style='color: green;'>Novo link enviado com sucesso. Acesse a sua caixa de e-mail para recuperar sua senha!</p>";
            $this->result = true;
        } else {
            $this->fromEmail = $sendEmail->getFromEmail();
            $_SESSION['msg'] = "<p style='color: #f00;'>Erro: E-mail com as instruções para recuperar a senha não enviado, tente novamente ou entre em contato com o e-mail {$this->fromEmail}!</p>";
            $this->result = false;
        }
    }
